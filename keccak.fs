...
: theta2 ( -- )
	...
	st i cells + 25 cells bounds DO  dup I @ xor I !  [ 5 cells ]L +LOOP
	drop
    LOOP ;
...
: chi ( -- )
	...
	I bc 5 cells move
	5 0 DO
	    bc I 1+ 5 mod cells + @ bc 2 + 5 mod cells + @ and
	    J I cells + dup >r @ xor r> !
	LOOP
    [ 5 cells ]L +LOOP ;

: iota ( round -- )
    cells keccakf_rndc + @ st @ xor st ! ;

: keccakf ( rounds -- )
    0 ?DO  theta1  theta2  rhopi  chi  I iota  LOOP ;
