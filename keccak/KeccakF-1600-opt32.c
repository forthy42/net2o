...
extractLanes(int byteCount, UINT32* state, UINT8* data) \
{
  int i;
  UINT64 tmp=0;
  for(i=0; i<(byteCount-7); i+=8) {
...
void xorInterleavedLE(int byteCount, UINT32* state, UINT32* input)
{
...
{
  xorInterleavedLE(byteCount, (UINT32*)state, input);
}
