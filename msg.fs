...
: ?search-lock ( addr u -- )
    BEGIN  dup  WHILE
		...
		2dup + $@ ['] msg:display catch-nobt IF  2drop  THEN
...
: msg-tredisplay ( n -- )
    ...
    [: cells >r msg-log@ { log u } u r> - 0 max { u' }
	  ...
	  I log - cell/ to log#
	  I $@ { d: msgt }
	  msgt ['] msg:display catch-nobt IF  ." invalid entry" cr
	      2drop  THEN
      cell +LOOP
...
: chain$, ( msgaddr u -- )
	...
	c:0key sigonly@ >hash hashtmp hash#128 forth:type ;] $tmp $, ;
: push, ( -- )
    push[] [: $, nestsig ;] $[]map ;
: ihave>push ( -- )
    ihave$ $@ bounds U+DO
...
: (send-avalanche) ( xt -- addr u flag )
    [:  msg-group-o .msg:?silent [: <msg IF msg-silent-start ELSE msg-start THEN
...
: send-like ( xc msg-addr u -- )
    [: chain$, msg-chain ulit, msg-like ;] send-avalanche ;
: execute-silent ( ... xt -- )
